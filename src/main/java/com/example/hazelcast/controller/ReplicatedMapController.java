...
public class ReplicatedMapController {
    ...
    @GetMapping(value = "/getReplicated/user/by/id/{id}")
    public ResponseEntity<ResponseWithExecutionTime> getUserByIdFromReplic(@PathVariable(value = "id") Long id){
        return processRequest(() -> userService.getFromReplicated(id).orElse(new User()));
    }
